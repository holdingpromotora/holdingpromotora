...
// POST - Inserir um tipo de acesso específico
export async function POST(request: NextRequest) {
  try {
    const body = await request.json();
    const { nome, descricao, nivel, cor, icone } = body;

    console.log(`🚀 Inserindo: ${nome} (nível ${nivel})`);

    const { data, error } = await supabase
      .from('tipos_acesso')
      .insert([{ nome, descricao, nivel, cor, icone }])
      .select()
      .single();
...
// GET - Inserir dados padrão um por vez
